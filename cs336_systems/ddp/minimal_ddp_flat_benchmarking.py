...
ARTIFACTS_DIR = "artifacts/minimal_ddp_flat_benchmarking"
...
def all_reduce_parameters(model):
    ...
    for p, grad_reduce in zip(model.parameters(), grads_reduced):
        if p.grad is not None:
            p.grad.data.copy_(grad_reduce)
    
    return end - start  # return time taken for all-reduce
...
def train(rank, world_size, backend, model_cfg, vocab_size, return_dict):
    ...
    print(f"Rank {rank}: Broadcast time: {(end_broadcast - start_broadcast)*1000:.2f} ms")
    ...
    for _ in range(5):
        for i in range(0, inputs.size(0), batch_size):
            ...
            comm_time = all_reduce_parameters(ddp_model)
...
if __name__ == "__main__":
    ...
    for rank in range(args.world_size):
        ...
        all_broadcast_times.append(result["broadcast_time_ms"])
    ...
    broadcast_stats = compute_stats(all_broadcast_times)
    ddp_step_stats = compute_stats(all_ddp_step_times)
    ddp_comm_stats = compute_stats(all_ddp_comm_times)
    
    print("Broadcast Time Stats (ms):", broadcast_stats)
